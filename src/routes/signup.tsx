...
import { prepareSignup } from "@/lib/referral.functions";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { toast } from "sonner";
import { MessageSquarePlus, Gift, Apple } from "lucide-react";
    ...
    <div className="flex min-h-screen items-center justify-center bg-muted/30 px-4">
      <div className="w-full max-w-md">
        ...
        <div className="rounded-lg border border-border bg-card p-8 shadow-sm">
          ...
          <p className="mt-1 text-sm text-muted-foreground">Gratuit. 3 projets inclus.</p>

          {ref && (
            <div className="mt-4 flex items-center gap-2 rounded-md bg-primary/10 border border-primary/20 px-3 py-2 text-sm">
              <Gift className="h-4 w-4 text-primary shrink-0" />
              <span>
                Parrainé par <strong className="font-mono">{ref.toUpperCase()}</strong> — 1 mois Pro offert à l'inscription.
              </span>
            </div>
          )}
          ...
          <Button onClick={handleApple} variant="outline" className="mt-2 w-full" type="button">
          ...
          <form onSubmit={handleSignup} className="space-y-4">
            <div>
              ...
              <Input id="fullName" type="text" required value={fullName} onChange={(e) => setFullName(e.target.value)} className="mt-1" />
            </div>
            <div>
